...
for m in st.session_state.schedule:
    ...
    with c2:
        ...
        st.markdown("<span style='background:{}; color:white; padding:4px 8px; border-radius:12px; font-weight:600;'>{}</span>".format(badge_color, status.title()), unsafe_allow_html=True)
    with c3:
        if not taken:
...
# Rewards: emoji by default; Turtle only if explicitly enabled and available
